      ...
      <CardHeader className="pb-4">
        <div className="flex items-center justify-between">
          <CardTitle
            className={`${
              isFeatured
                ? "text-2xl font-bold text-warm-gray-900"
                : "text-xl font-semibold text-warm-gray-800"
            } leading-tight`}
          >
            {title}
          </CardTitle>
          <Badge
            className={`text-white font-semibold px-3 py-1 ${
              isFeatured
                ? "bg-bordeaux-600 text-base"
                : "bg-bordeaux-500 text-sm"
            }`}
          >
            ab {startingPrice}
          </Badge>
  ...
  return (
    <Card
      className={`h-full rounded-xl ${
        isFeatured
          ? "border-t-8 border-t-bordeaux-600"
          : "border-t-4 border-t-bordeaux-500"
      } shadow-md hover:shadow-xl hover:-translate-y-1 transition-all duration-300 ease-out overflow-hidden`}
    >
      {isFeatured && image ? (
        <div className="grid md:grid-cols-2 h-full">
          ...
          <div className="flex flex-col p-6">{cardContent}</div>
        </div>
      ) : (
        <div className="flex flex-col flex-1 h-full p-6">{cardContent}</div>
      )}
    </Card>
  );
}
